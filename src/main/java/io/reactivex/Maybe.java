...
public abstract class Maybe<T> implements MaybeSource<T> {
    ...
    public static <T> Maybe<T> amb(@NonNull final Iterable<? extends MaybeSource<? extends T>> sources) {
    ...
    public static <T> Maybe<T> ambArray(@NonNull final MaybeSource<? extends T>... sources) {
        if (sources.length == 0) {
            return empty();
        }
        if (sources.length == 1) {
